...
// прога=поиск в отсортированном списке идентификационных номеров клиентов и выводит соответствующий кредитный баланс
...
{
nums[ctr] = (rand() % 99) + 1; // функция rand() возвращает случайное число от 0 до максимального значения
}
...
//! ЭТО прога с книги. Фишка - во вложенном в оба массива счётчке ctr - он делает оба массива параллельными.
/*int ctr;       //Счетчик цикла
  int idSearch;  //Искомый клиент (ключ)
  ...
  float custBal[10]= { 0.00, 45.43, 71.23, 301.56, 9.08, 192.41, 
                      389.00, 229.67, 18.31, 59.54};  // Взаимодействие с пользователем, ищущим баланс
  printf("\n\n*** Проверка баланса клиента ***\n");
  printf("Введите искомый номер клиента: ");
  scanf(" %d", &idSearch);  // Поиск и установление наличия номера клиента в массиве
  for (ctr = 0; ctr < 10; ctr++){
...
printf("Enter indefinite number of customers: ");
scanf("%d", &CustID);
...
dollars);
//! Абсолютное значение выводится без дробной части благодаря использованию кода
преобразования %f со спецификатором .0. printf("Абсолютное значение -25.0:
%.0f.\n", fabs(-25.0));
